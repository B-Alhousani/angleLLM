"""
Evaluation script for DepthLM (facebook/DepthLM) with CSV Reporting.
FIXES APPLIED:
 - Switched to AutoModelForVision2Seq (Fixes 'llava' vs 'qwen' mismatch)
 - Added manual casting of pixel_values to BFloat16 (Fixes 'Input type' crash)
 - Removed 'image_sizes' argument (Fixes 'unused model_kwargs' crash)
...
def main(args):
    ...
    try:
        ...
        # Safe to keep padding side fix, usually harmless for LLaVA
        processor.tokenizer.padding_side = "left"
    except Exception as e:
        print(f"Error loading processor: {e}")
        return

    # 2. Load Model (Using AutoModelForVision2Seq to handle LLaVA/Qwen automatically)
    try:
        ...
        with open(args.output_csv, mode='a', newline='', encoding='utf-8') as f:
            ...
            for batch in chunks(samples_to_run, args.batch_size):
                ...
                try:
                    ...
                    # 2. CRITICAL FIX: Convert Float32 inputs to BFloat16 to match model weights
                    # This fixes: "Input type (torch.cuda.FloatTensor) and weight type (CUDABFloat16Type) should be the same"
                    for k, v in inputs.items():
                        if torch.is_floating_point(v):
                            inputs[k] = v.to(dtype=torch.bfloat16)
                    
                    # 3. CRITICAL FIX: Remove 'image_sizes' if present
                    # This fixes: "The following model_kwargs are not used by the model: ['image_sizes']"
...
if __name__ == "__main__":
    ...
    p.add_argument("--model_path", default="facebook/DepthLM")
    p.add_argument("--eval_jsonl", default="/mnt/dgx_lab/datasets/IbimsOG/31859_dataset_metadata.jsonl")
    p.add_argument("--output_csv", default="evaluation_results_hugging.csv")
