...
def scale_image_by_factor(image: Image, intrinsics: list, scale_factor: float):
    """
    Scales the image and its intrinsics by a multiplication factor.
    Replaces the old normalizing_focal_length function.
    """
    if scale_factor == 1.0:
        # Even if factor is 1.0, we must ensure the list has 6 elements (adding w/h)
        # because the rest of the pipeline might expect it.
        if len(intrinsics) < 6:
             return image, intrinsics + [image.width, image.height]
        return image, intrinsics
    ...
    # We create a NEW list to ensure we have exactly 6 elements [fx, fy, cx, cy, w, h]
    intrinsics_new = [
        intrinsics[0] * scale_factor,  # fx
        intrinsics[1] * scale_factor,  # fy
        intrinsics[2] * scale_factor,  # cx
        intrinsics[3] * scale_factor,  # cy
        new_width,                     # width (index 4)
        new_height                     # height (index 5)
    ]
...
def draw_visual_intrinsics(image: Image, intrinsics: list, target_coords: tuple) -> Image:
    """
    Draws yellow angle grids and a red target ring on the image to visualize intrinsics.
    """
    overlay = np.array(image)
    
    fx, fy, cx, cy = intrinsics[:4]
    w, h = image.size
    target_u, target_v = target_coords
...
class dataset_eval(Dataset):
    def __init__(
        ...
        fixed_scale_factor=1.0, # changed from normalized_focal_length
    ) -> None:
    ...
    def extract_image_and_meta(self, index):
        ...
        # APPLY FIXED SCALING instead of normalization
        data_dict["image"], intrinsics_new = scale_image_by_factor(
            data_dict["image"], intrinsics_new, self.fixed_scale_factor
        )
        ...
        if (
            ...
            # --- ADD GRID LAYERS ---
            data_dict["image"] = draw_visual_intrinsics(
                data_dict["image"], 
                intrinsics_new, 
                (scaled_pixel_x, scaled_pixel_y)
            )
            ...
            for dy in range(1, cross_size // 2 + 1):
                data_dict["image"].putpixel(
                    (scaled_pixel_x - dy - 1, scaled_pixel_y + dy), (255, 0, 0)
                )
                data_dict["image"].putpixel(
                    (scaled_pixel_x - dy - 1, scaled_pixel_y - dy), (255, 0, 0)
                )
...
class dataset_train(Dataset):
    def __init__(
        ...
        width_max=1400,
        width_min=1000,
        focal_length_range=(0.8, 1.2), # changed from normalized_focal_length
        sample_weights=None, 
        ratio_min=1.0, 
        ratio_max=1.3,
    ) -> None:
    ...
    def __len__(self, ori_length=False) -> int:
        if ori_length:
            ...
            for data_dict in self.list_data_dict:
                length += len(data_dict)
            return length * 2 # Double the length for original + augmented
        else:
            length = 0
            for data_dict in self.list_data_dict:
                length += (
    ...
    def getitem_Taskonomy(
        ...
        # Determine scaling logic based on index even/odd
        is_augmented = (index % 2 != 0)
        
        if is_augmented:
            scale_factor = random.uniform(self.focal_length_range[0], self.focal_length_range[1])
        else:
            scale_factor = 1.0

        # Adjust index to fit original dataset bounds
        # We process index // 2 basically, but strictly we just need wrapping
        ...
        # APPLY SCALING
        data_dict["image"], intrinsics_new = scale_image_by_factor(
            data_dict["image"], intrinsics_new, scale_factor
        )

        # Taskonomy specific aspect ratio noise (preserved as requested)
        new_height = int(
        ...
        intrinsics_new[1] *= new_height / intrinsics_new[5]
        intrinsics_new[3] *= new_height / intrinsics_new[5]
        intrinsics_new[5] = new_height
    ...
    def getitem_noTaskonomy(self, index, id_dataset):
        ...
        # APPLY SCALING
        img, intrinsics_new = scale_image_by_factor(
            img, intrinsics_new, scale_factor
        )
        ...
        crop_height = int(min(height, random.uniform(self.height_range[0], self.height_range[1])))
        crop_width = int(min(width, random.uniform(self.width_range[0], self.width_range[1])))
        ...
        intrinsics_new[2] -= left
        intrinsics_new[3] -= top
        intrinsics_new[4] = data_dict["image"].width
        intrinsics_new[5] = data_dict["image"].height

        pixel_coords = [
            ([coord[0] - left, coord[1] - top] if is_within_range(coord, (left, top, right, bottom)) else [-1, -1])
            for coord in pixel_coords
        ]

        if len(self.list_data_dict[id_dataset][index]["pixel_coords"]) - 1 > 0:
            random_index = random.randint(0, len(self.list_data_dict[id_dataset][index]["pixel_coords"]) - 1)
        else:
            print("no pixel in ", index, ": ", self.list_data_dict[id_dataset][index])
