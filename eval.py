...
def main(args):
    if "image_sizes" in args:
        args.pop("image_sizes")
        print("args removed")
    ...
    dataset = dataset_eval(
        json_path, img_fodler, normalized_focal_length = 1000.0 # change to the corresponding value for other models
    )
    ...
    with torch.no_grad():
        ...
        for i in tqdm(range(0, len(sampled_indices), args.bsz)):
            ...
            if "pixtral" in model_path.lower():
                ...
                if getattr(args, 'apply_system_prompt', False):
                    text = [
