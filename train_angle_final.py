...
from utils.angle_dataset import DatasetTrain
...
class ModelConfig(trl.ModelConfig):
    ...
    # ADDED: explicitly define torch_dtype so the parser recognizes it
    torch_dtype: Optional[str] = field(
        default="bfloat16", metadata={"help": "The dtype to use for the model."}
    )
...
class SFTConfig(trl.SFTConfig):
    ...
    # ADDED: explicitly define max_seq_length so the parser recognizes it
...
def main(script_args, training_args, model_args):
    ...
    dataset = DatasetTrain(script_args.dataset_name, script_args.image_folder, **dataset_kwargs)
    ...
    # supporting qwen only, this need to be changed if tested on another models
